...
void main() {
    // Get the world position of the card instance center
    vec3 centerPos = (uWorldMatrix * aInstanceMatrix * vec4(0., 0., 0., 1.)).xyz;

    // Project ONLY the center position onto the virtual sphere
    float radius = length(centerPos);
    vec3 projectedCenter = radius * normalize(centerPos);

    // Compute local model vertex offset relative to instance center
    vec4 worldPosition = uWorldMatrix * aInstanceMatrix * vec4(aModelPosition, 1.);
    vec3 localOffset = worldPosition.xyz - centerPos;

    // Apply rotation stretch to local offsets
    if (gl_VertexID > 0) {
        vec3 rotationAxis = uRotationAxisVelocity.xyz;
        float rotationVelocity = min(.15, uRotationAxisVelocity.w * 15.);
        vec3 stretchDir = normalize(cross(centerPos, rotationAxis));
        vec3 relativeVertexPos = normalize(localOffset);
        float strength = dot(stretchDir, relativeVertexPos);
        float invAbsStrength = min(0., abs(strength) - 1.);
        strength = rotationVelocity * sign(strength) * abs(invAbsStrength * invAbsStrength * invAbsStrength + 1.);
        localOffset += stretchDir * strength;
    }

    // Combine flat local offset with projected center
    worldPosition.xyz = projectedCenter + localOffset;

    gl_Position = uProjectionMatrix * uViewMatrix * worldPosition;

    // Calculate alpha based on front-facing direction on virtual sphere
    vAlpha = smoothstep(0.5, 1., normalize(projectedCenter).z) * .9 + .1;
    vUvs = aModelUvs;
...
class RoundedRectangleGeometry extends Geometry {
  constructor(w = 1.0, h = 1.25, r = 0.22, stepsPerCorner = 10) {
    super();
    ...
    // 2. Corner definitions (cx, cy) and arc sweeps
    const corners = [
      { cx: w / 2 - r, cy: h / 2 - r, startAngle: 0, endAngle: Math.PI / 2 }, // Top-Right
      { cx: -w / 2 + r, cy: h / 2 - r, startAngle: Math.PI / 2, endAngle: Math.PI }, // Top-Left
      { cx: -w / 2 + r, cy: -h / 2 + r, startAngle: Math.PI, endAngle: 3 * Math.PI / 2 }, // Bottom-Left
      { cx: w / 2 - r, cy: -h / 2 + r, startAngle: 3 * Math.PI / 2, endAngle: 2 * Math.PI } // Bottom-Right
    ];

    for (let c = 0; c < 4; c++) {
      const { cx, cy, startAngle, endAngle } = corners[c];
      for (let i = 0; i < stepsPerCorner; i++) {
        ...
        // Clean UV Mapping for rectangular geometry
        this.lastVertex.uv[0] = x / w + 0.5;
        this.lastVertex.uv[1] = y / h + 0.5;

        totalVertices++;
      }
    }

    // 3. Connect outer perimeter vertices to the center
    for (let idx = 1; idx < totalVertices; idx++) {
      const nextIdx = idx === totalVertices - 1 ? 1 : idx + 1;
      this.addFace(0, idx, nextIdx);
    }
  }
}
